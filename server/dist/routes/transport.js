...
router.post('/auto-assign', async (req, res) => {
    try {
        ...
        if (!order) {
            const memOrder = global.inMemoryOrders?.find((o) => o._id === orderId);
            if (!memOrder) {
        ...
        try {
            ...
            const plantDestinations = plants.map((plant) => ({
                name: plant.name,
                id: plant._id,
            ...
            if (destLat && destLng) {
                ...
                if (mlResponse.data?.selected_plant) {
                    ...
                    selectedPlant = plants.find((p) => p._id === selectedPlantId || p.name === mlResponse.data.selected_plant.plant_name);
        ...
        if (!selectedPlant) {
            ...
            try {
                ...
                const origins = plants.map((p) => `${p.location.coordinates.lat},${p.location.coordinates.lng}`).join('|');
                const apiKey = process.env.GOOGLE_MAPS_API_KEY;
                if (apiKey) {
        ...
        if (!vehicle) {
            ...
            const quantity = order?.product?.quantity || order?.quantity || 100;
            vehicle = {
        ...
        if (order.save && typeof order.save === 'function') {
            ...
            const index = global.inMemoryOrders?.findIndex((o) => o._id === orderId);
