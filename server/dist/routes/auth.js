...
router.post('/signup', async (req, res) => {
    try {
        ...
        try {
            ...
            await user.save();
            // Generate token
            const token = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_SECRET || 'your-secret-key', { expiresIn: process.env.JWT_EXPIRES_IN || '7d' });
...
router.get('/me', async (req, res) => {
    try {
        ...
        try {
            ...
            if (global.inMemoryUsers) {
                const user = global.inMemoryUsers.find((u) => u._id === decoded.id);
                if (user) {
