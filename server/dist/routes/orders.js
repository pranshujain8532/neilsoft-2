...
router.get('/', async (req, res) => {
    try {
        ...
        try {
            ...
            if (customerId) {
                orders = orders.filter((o) => o.customer === customerId || o.customer?._id === customerId);
            }
            if (status) {
            ...
            orders = orders.map((o) => {
                if (global.inMemoryUsers) {
                    const customer = global.inMemoryUsers.find((u) => u._id === o.customer || u._id === o.customer?._id);
                    if (customer) {
...
router.get('/:id', async (req, res) => {
    try {
        try {
            ...
            // Check in-memory
            const order = global.inMemoryOrders.find((o) => o._id === req.params.id);
            if (order) {
                // Mock populate
                if (global.inMemoryUsers) {
                    const customer = global.inMemoryUsers.find((u) => u._id === order.customer || u._id === order.customer?._id);
                    if (customer) {
...
router.put('/:id/status', async (req, res) => {
    try {
        try {
            const order = await Order.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true }).maxTimeMS(2000);
            if (order)
                return res.json(order);
        }
        catch (dbError) {
            // Update in-memory
            const index = global.inMemoryOrders.findIndex((o) => o._id === req.params.id);
            if (index !== -1) {
