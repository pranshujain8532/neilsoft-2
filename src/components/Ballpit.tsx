// Ballpit component providing an interactive three.js background
...
class X {
    ...
    #adjustFov(aspect: number) {
        const tanFov = Math.tan((this.cameraFov * Math.PI) / 360);
        const newTan = tanFov / (this.camera.aspect / aspect);
        this.camera.fov = 2 * (Math.atan(newTan) * 180) / Math.PI;
    }
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): { three: X; spheres: Z; setCount: (count: number) => void; togglePause: () => void; dispose: () => void } {
