...
class StorageAnomalyDetector:
    """
    LSTM Autoencoder for detecting anomalies in storage sensor readings.
    
    Architecture:
    - Encoder: LSTM layers compress the input sequence
    ...
    def __init__(self, model_path: str = 'models/saved/storage_anomaly_lstm.h5'):
        ...
        self.sequence_length = 10  # Time steps to consider
        self.n_features = 4  # pressure, temperature, level, purity
    ...
    def fetch_training_data(self, days: int = 30) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for container in containers:
                ...
                # Fetch sensor readings for this container
                ...
                # Group readings by timestamp and pivot sensor types
                time_groups = {}
                for r in readings:
                    ts = r['timestamp']
                ...
                # Convert to array format [pressure, temperature, level, purity]
                data_points = []
    ...
    def generate_synthetic_data(self, n_samples: int = 5000) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Generate normal operating data (80%)
        for _ in range(int(n_samples * 0.8)):
            ...
            for t in range(self.sequence_length):
                # Small random walk (normal variation)
            ...
            for t in range(self.sequence_length):
                if t >= anomaly_start:
                    if anomaly_type == 'pressure_spike':
                        pressure += np.random.uniform(5, 15)  # Sudden increase
                    elif anomaly_type == 'temp_anomaly':
                        temperature += np.random.uniform(3, 10)  # Overheating
                    elif anomaly_type == 'leak':
                        level -= np.random.uniform(2, 5)  # Rapid level drop
                    elif anomaly_type == 'purity_drop':
                        purity -= np.random.uniform(0.1, 0.5)  # Contamination
                else:
                    # Normal variation before anomaly
                    pressure += np.random.normal(0, 0.5)
                    temperature += np.random.normal(0, 0.2)
                    level += np.random.normal(0, 0.3)
                    purity += np.random.normal(0, 0.001)
    ...
    def train(self, epochs: int = 50, batch_size: int = 32) -> Dict:
        ...
        if real_data is not None and len(real_data) > 100:
            ...
            X_train = X_normal  # Train only on normal data (autoencoder paradigm)
    ...
    def detect_anomaly(self, sensor_data: Dict) -> Dict:
        """
        Detect anomaly in sensor readings for a container.
        
        Args:
            sensor_data: Dict with container_id and recent readings
        
        Returns:
            Dict with anomaly score, is_anomaly flag, and details
        """
        if not HAS_TF or self.model is None:
            return self._fallback_detection(sensor_data)
        
        try:
            # Get recent readings for the container
            ...
            # Identify which sensor is anomalous
    ...
    def _fetch_recent_readings(self, container_id: str) -> List[List[float]]:
        """Fetch recent sensor readings for a container"""
        try:
    ...
    def _create_alert(self, container_id: str, result: Dict):
        """Create an alert in Supabase when anomaly is detected - only if no existing unresolved alert"""
        try:
            # Check for existing unresolved alerts for this container
    ...
    def save_model(self):
        ...
        # Save scaler params and threshold
