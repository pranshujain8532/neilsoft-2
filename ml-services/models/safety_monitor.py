...
class SafetyMonitor:
    def __init__(self, model_path='models/saved/safety_pinn.h5'):
        ...
        # Safety thresholds
        self.thresholds = {
            'pressure': {'min': 1, 'max': 30, 'critical': 35},  # bar
            'temperature': {'min': 15, 'max': 80, 'critical': 90},  # °C
            'flow_rate': {'min': 0.5, 'max': 50, 'critical': 60},  # kg/s
            'current': {'min': 100, 'max': 5000, 'critical': 6000},  # A
        }
    ...
    def build_model(self):
        ...
        # Input: pressure, temperature, flow_rate, current
        ...
        # Output: safety score (0-1) and anomaly detection
    ...
    def generate_training_data(self, num_samples=2000):
        ...
        for _ in range(num_samples):
            ...
            if np.random.random() < 0.8:
                pressure = np.random.uniform(5, 25)  # bar
                temperature = np.random.uniform(25, 70)  # °C
                flow_rate = np.random.uniform(5, 40)  # kg/s
                current = np.random.uniform(500, 4000)  # A
                
                # Physics constraint: P * V = n * R * T (ideal gas approximation)
                # Higher pressure and temp should correlate
    ...
    def check_safety(self, machine_data: Dict) -> Dict:
        """Check safety status of machine"""
